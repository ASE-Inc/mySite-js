...
var mySite = $.extend(true, {
  ...
  calcLevelChange: function (current, previous) {
    ...
    if(current.Levels[current.pageChangeLevel + 1] != previous.Levels[current.pageChangeLevel + 1]) break;
  },
  ajaxPageLoader: null,
  ajaxTo: null,
  twitter_streams: {},
    ...
    function Tweet(data) {
      ...
      this.loadOEmbed = function () {
        if(Modernizr.webworkers) {
          ...
          this.worker.addEventListener('error', function (event) {}, false);
      ...
      this.update = function () {
        $.getJSON("https://twitter.com/status/user_timeline/" + THIS.username + ".json?count=9&since_id=" + THIS.tweets[0].id + "&callback=?", function (data) {
          $.each(data, function (index, tweet) {
            if(tweet != THIS.tweets[i].data) THIS.tweets.unshift(new Tweet(tweet));
          });
        });
      }
      this.load = function () {
  ...
  if(Modernizr.canvas) {
    ...
    $.fn.paintParticles = function (o) {
      ...
      return this.each(function () {
        ...
        THIS.particleWorker = new ParticleWorker(function (event) {
          switch(event.data.type) {
            ...
            default:
              ...
              if(THIS.particles.length > 5999) THIS.particles.splice(1000);
              break;
          }
        });
        ...
        var ab = new Uint8Array(1).buffer,
          transferableSupported;
        try {
        ...
        var draw = function () {
          ...
          if(popped = THIS.particles.pop()) THIS.drawParticles(popped, context);
          //drawing=false;
          fps++;
          if(THIS.particles.length == 75 || THIS.particles.length == 3) THIS.particleWorker.worker.postMessage({
            action: "resume"
          });
          //}
    ...
    $.fn.paintTwinklingStars = function (o) {
      var x, y, r, newo, twoPI = Math.PI * 2,
        len, pLen;
    ...
    $.fn.ajaxLinks = function (o) {
      ...
      return this.each(function () {
        $(this).on('click', function (e) {
          ...
          if(link.tagName.toUpperCase() !== 'A') throw "requires an anchor element";
